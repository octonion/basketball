...
r.shot_clock as shot_clock,
ln(r.team_score::float) as log_ps
from ncaa_sr.results r
where
TRUE
and r.year between 1994 and 2015
and r.team_score>0
...
fp <- data.frame(year,field,game_length,shot_clock)
fpn <- names(fp)
...
#model1 <- log_ps ~ year*field+game_length+(1|offense)+(1|defense)+(1|game_id)
#fit1 <- lmer(model1, data=g, REML=FALSE)

model <- log_ps ~ year+field+game_length+shot_clock+(1|offense)+(1|defense)+(1|game_id)
