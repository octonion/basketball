...
from ncaa_women._parameter_levels_geocoding
where parameter='year'
);

create table if not exists ncaa_women._factors_geocoding (
       parameter		text,
       level			text,
       type			text,
       method			text,
...
truncate table ncaa_women._factors_geocoding;
...
insert into ncaa_women._factors_geocoding
...
from ncaa_women._parameter_levels_geocoding npl
left outer join ncaa_women._basic_factors_geocoding nbf
...
parameter,
avg(raw_factor)
from ncaa_women._factors_geocoding
where parameter not in ('o_div','d_div')
group by parameter
);

update ncaa_women._factors_geocoding
set raw_factor=raw_factor-s.mean
from scale s
where s.parameter=_factors_geocoding.parameter;
