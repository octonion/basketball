...
con <- dbConnect(drv,host="localhost",port="5432",dbname="rockets")
...
dbWriteTable(con,c("euroleague","_parameter_levels"),parameter_levels,row.names=TRUE)
...
dbWriteTable(con,c("euroleague","_basic_factors"),as.data.frame(combined),row.names=TRUE)
