...
from euroleague._parameter_levels
where parameter='year'
);

drop table if exists euroleague._factors;

create table euroleague._factors (
       parameter		text,
       level			text,
       type			text,
       method			text,
...
insert into euroleague._factors
...
parameter,
avg(raw_factor)
from euroleague._factors
where parameter not in ('o_div','d_div')
group by parameter
);

update euroleague._factors
set raw_factor=raw_factor-s.mean
from scale s
where s.parameter=euroleague._factors.parameter;
*/
