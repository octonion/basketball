...
from bbref._parameter_levels
where parameter='year'
);

drop table if exists bbref._factors;

create table bbref._factors (
       parameter		text,
       level			text,
       type			text,
       method			text,
...
from bbref._parameter_levels npl
left outer join bbref._basic_factors nbf
