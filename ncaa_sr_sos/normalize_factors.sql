...
from ncaa_sr._parameter_levels
where parameter='year'
);

drop table if exists ncaa_sr._factors;

create table ncaa_sr._factors (
       parameter		text,
       level			text,
       type			text,
       method			text,
...
from ncaa_sr._parameter_levels npl
left outer join ncaa_sr._basic_factors nbf
...
from scale s
where s.parameter=ncaa_sr._factors.parameter;
*/
...
null as first_year,
null as last_year,
avg(bf.raw_factor) as raw_factor,
exp(avg(bf.raw_factor)) as factor
from ncaa_sr._factors bf
where
