...
psql basketball -f schema/create_schema_euroleague.sql

tail -q -n+2 csv/teams_*.csv > /tmp/teams.csv
psql basketball -f loaders/load_euroleague_teams.sql
rm /tmp/teams.csv

tail -q -n+2 csv/games_*.csv > /tmp/games.csv
psql basketball -f loaders/load_euroleague_games.sql
rm /tmp/games.csv
