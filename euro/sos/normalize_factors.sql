...
from euro._parameter_levels
where parameter='year'
);

drop table if exists euro._factors;

create table euro._factors (
       parameter		text,
       level			text,
       type			text,
       method			text,
...
update euro._factors
set raw_factor=raw_factor-s.mean
from scale s
where s.parameter=euro._factors.parameter;
*/
