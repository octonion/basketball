...
from eurocup._parameter_levels
where parameter='year'
);

drop table if exists eurocup._factors;

create table eurocup._factors (
       parameter		text,
       level			text,
       type			text,
       method			text,
...
from eurocup._parameter_levels npl
left outer join eurocup._basic_factors nbf
...
parameter,
avg(raw_factor)
from eurocup._factors
where parameter not in ('o_div','d_div')
group by parameter
);

update eurocup._factors
set raw_factor=raw_factor-s.mean
from scale s
where s.parameter=eurocup._factors.parameter;
*/
